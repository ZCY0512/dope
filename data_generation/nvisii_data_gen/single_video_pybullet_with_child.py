...
def adding_mesh_object(
    ...
    if texture_to_load is None:
        ...
        toy.get_material().set_roughness(0.9)
...
def add_child_object(parent_name, child_name, child_obj_path, child_texture_path=None, scale=1, offset_position=(0,0,0), offset_rotation=(0,0,0,1)):
    global mesh_loaded
    # Load the mesh and texture for the child object
    if child_texture_path is None:
        toys = load_obj_scene(child_obj_path)
        if len(toys) > 1:
        ...
        child_obj = visii.entity.create(
            ...
            material=toy_material,
            mesh=visii.mesh.create_from_file(child_name, child_obj_path),
        )

        toy_transform = child_obj.get_transform()
        child_obj.get_material().set_roughness(random.uniform(0.1, 0.5))

        for toy in toys:
            visii.entity.remove(toy)

    else:
        if child_obj_path in mesh_loaded:
            toy_mesh = mesh_loaded[child_obj_path]
        else:
            toy_mesh = visii.mesh.create_from_file(child_name, child_obj_path)
            mesh_loaded[child_obj_path] = toy_mesh
        ...
        child_rgb_tex = visii.texture.create_from_file(child_name, child_texture_path)
        child_obj.get_material().set_base_color_texture(child_rgb_tex)
        child_obj.get_material().set_roughness(random.uniform(0.1, 0.5))

        toy_transform = child_obj.get_transform()

    # Set the child's transform relative to the parent
    parent_transform = visii.entity.get(parent_name).get_transform()
    toy_transform.set_parent(parent_transform)

    # Set the offset position and rotation for the child object
    toy_transform.set_scale(visii.vec3(scale))
    toy_transform.set_position(visii.vec3(*offset_position))
    toy_transform.set_rotation(visii.quat(*offset_rotation))
...
if opt.path_single_obj is not None:
    for i_object in range(opt.nb_objects):
        ...
        adding_mesh_object(f"single_obj_{i_object}",
                           opt.path_single_obj,
                           "/home/chengyan/disk2/DART/postprocess/data/hand_texture.png",
                           model_info_path,
                           scale=opt.scale,
                           debug=opt.debug,
                           )
        # Add a child object
        parent_name = f"single_obj_{i_object}"
        child_name = f"single_obj_child_{i_object}"
        child_obj_path = opt.path_single_child
        child_texture_path = None
        scale = 1
        offset_position = (0, 0, 0) 
        offset_rotation = (1, 0, 0, 0)

        add_child_object(parent_name, child_name, child_obj_path, child_texture_path, scale, offset_position, offset_rotation)
