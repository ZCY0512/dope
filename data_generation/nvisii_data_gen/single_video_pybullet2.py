...
distractors_folder="model_distractor"
dexycb_folder=glob.glob(distractors_folder+"/*")
for i_obj in range(int(opt.nb_distractors)):

    toy_to_load = dexycb_folder[random.randint(0,len(dexycb_folder)-1)]
    obj_to_load = toy_to_load + "/textured.obj"
    texture_to_load = toy_to_load + "/texture_map.png"
    name = "dexycb_"+toy_to_load.split('/')[-2] + f"_{i_obj}"
...
if opt.path_single_obj is not None:
    for i_object in range(opt.nb_objects):
        model_info_path = os.path.dirname(opt.path_single_obj) + '/textured.obj.json'
        texture_to_load_interact=os.path.dirname(opt.path_single_obj) + '/texture_map.png'
        if not os.path.exists(model_info_path):
            model_info_path = None

        adding_mesh_object(f"single_obj_{i_object}",
                           opt.path_single_obj,
                           texture_to_load_interact,
                           model_info_path,
                           scale=opt.scale,
                           debug=opt.debug)
        #adding_mesh_object(f"single_obj_{i_object}", os.path.dirname(opt.path_single_obj)+'/textured.obj', os.path.dirname(opt.path_single_obj)+'/texture_map.png', model_info_path,debug=opt.debug)
